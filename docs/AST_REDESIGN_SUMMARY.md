...
1. Dirty Range Pipeline (минимальный): текстовые правки → диапазоны offset → фильтрация узлов по пересечению span → частичная перестройка AST подпоследовательности.
2. Fingerprint Slice Reuse: повторное вычисление fingerprint только для новых/затронутых узлов + переиспользование неизменённых поддеревьев (проверка через diff_changed_nodes).
3. Metric: `incr.reused_subtrees` (количество узлов, отпавших благодаря совпавшему fingerprint).
4. Benchmark сценарий: N мелких правок в одном файле; цель — повторный анализ без структурных изменений ≤30% времени полного.
5. Потенциальная оптимизация hash: переход на специализированный смешиватель (например, ahash) с фиксированным seed при необходимости ускорения.
