...
use super::entity::{
    BslConstraints, BslContext, BslEntity, BslEntityId, BslEntityKind, BslEntitySource,
    BslEntityType, BslInterface, BslLifecycle, BslProperty, BslRelationships, BslTabularSection,
};
...
impl ConfigurationXmlParser {
    ...
    pub fn parse_configuration(&self) -> Result<Vec<BslEntity>> {
        ...
        entities
            .extend(self.parse_metadata_objects_with_forms("Catalogs", BslEntityKind::Catalog)?);
        entities
            .extend(self.parse_metadata_objects_with_forms("Documents", BslEntityKind::Document)?);
        entities.extend(self.parse_metadata_objects_with_forms(
            "ChartsOfCharacteristicTypes",
            BslEntityKind::ChartOfCharacteristicTypes,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms(
            "ChartsOfAccounts",
            BslEntityKind::ChartOfAccounts,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms(
        ...
        entities.extend(self.parse_metadata_objects_with_forms(
            ...
            BslEntityKind::InformationRegister,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms(
        ...
        entities.extend(self.parse_metadata_objects_with_forms(
            "AccountingRegisters",
            BslEntityKind::AccountingRegister,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms(
            "CalculationRegisters",
            BslEntityKind::CalculationRegister,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms(
            "BusinessProcesses",
            BslEntityKind::BusinessProcess,
        )?);
        entities.extend(self.parse_metadata_objects_with_forms("Tasks", BslEntityKind::Task)?);
        entities.extend(
            self.parse_metadata_objects_with_forms("ExchangePlans", BslEntityKind::ExchangePlan)?,
        );
        ...
        entities.extend(
            self.parse_metadata_objects_with_forms("DataProcessors", BslEntityKind::DataProcessor)?,
        );

        // Парсим журналы документов с их формами
        entities.extend(self.parse_metadata_objects_with_forms(
            "DocumentJournals",
            BslEntityKind::DocumentJournal,
        )?);
    ...
    fn parse_metadata_objects_with_forms(
        &self,
        folder_name: &str,
        kind: BslEntityKind,
    ...
    fn parse_common_forms(&self) -> Result<Vec<BslEntity>> {
        ...
        for entry in fs::read_dir(&forms_path)? {
            ...
            if form_path.is_dir() {
                ...
                let form_entity =
                    self.parse_form_xml(&actual_form_xml, "ОбщиеФормы", &form_name)?;
    ...
    pub fn parse_metadata_object(
        &self,
        xml_path: &Path,
        kind: Option<&BslEntityKind>,
    ) -> Result<BslEntity> {
        let content = fs::read_to_string(xml_path).context("Failed to read metadata XML file")?;
                    ...
                    match tag_name.as_str() {
                        ...
                        "Type" if in_properties => {
                            ...
                            if entity.entity_kind == BslEntityKind::Constant {
                                if let Ok(type_name) =
                                    self.parse_constant_type(&mut reader, &mut buf)
                                {
                                    ...
                                    entity
                                        .interface
                                        .properties
                                        .insert("Значение".to_string(), prop);
                                }
                            }
                        }
                        "ChildObjects" => {
                        ...
                        "EnumValue" => {
                            ...
                            if !current_enum_name.is_empty() {
                                let enum_prop = BslProperty {
                                    ...
                                    is_readonly: true,
                                    is_indexed: false,
                                    documentation: if current_enum_synonym.is_empty() {
                                        None
                                    } else {
                                        Some(current_enum_synonym.clone())
                                    },
                                    availability: vec![BslContext::Client, BslContext::Server],
                                };
                                entity
                                    .interface
                                    .properties
                                    .insert(current_enum_name.clone(), enum_prop);
                            }
                            in_enum_value = false;
                        }
        ...
        entity.source = BslEntitySource::ConfigurationXml {
                ...
                if xml_path.exists() {
                    if let Ok(mut entity) = self.parse_metadata_object(&xml_path, Some(&kind)) {
                        ...
                        entity.qualified_name =
                            format!("{}.{}", self.get_kind_prefix(&kind), object_name);
                        ...
                        if forms_path.exists() {
                            for form_entry in fs::read_dir(&forms_path)? {
                                ...
                                let form_name =
                                    form_entry.file_name().to_string_lossy().to_string();
    ...
    fn get_parent_types(&self, kind: &BslEntityKind) -> Vec<String> {
        match kind {
            ...
            BslEntityKind::ChartOfCharacteristicTypes => {
                vec!["ПланВидовХарактеристикОбъект".to_string()]
            }
            ...
            BslEntityKind::AccumulationRegister => {
                vec!["РегистрНакопленияНаборЗаписей".to_string()]
            }
            ...
            match reader.read_event_into(buf) {
                ...
                Err(e) => {
                    return Err(anyhow::anyhow!(
                        "XML parsing error in tabular section: {}",
                        e
                    ))
                }
                _ => {}
    ...
    pub fn parse_object_forms(
        ...
        for entry in fs::read_dir(&forms_path)? {
            ...
            if form_path.is_dir() {
                ...
                let form_entity =
                    self.parse_form_xml(&actual_form_xml, parent_qualified_name, &form_name)?;
    ...
    fn parse_form_xml(
        ...
        let content = fs::read_to_string(form_xml_path).context("Failed to read form XML file")?;
        ...
        let mut form_entity = BslEntity {
            ...
            source: BslEntitySource::FormXml {
                path: String::new(),
            },
        ...
        let mut form_commands: Vec<String> = Vec::new();
        let mut form_items: Vec<String> = Vec::new();
            ...
            match reader.read_event_into(&mut buf) {
                Ok(Event::Start(ref e)) => {
                    ...
                    match tag_name.as_str() {
                        ...
                        "Table" | "CommandBar" | "Button" | "InputField" | "Field"
                        | "UsualGroup" | "Group" => {
                            for attr in e.attributes().flatten() {
                                let key = String::from_utf8_lossy(attr.key.as_ref()).to_string();
                                let value = String::from_utf8_lossy(&attr.value).to_string();
