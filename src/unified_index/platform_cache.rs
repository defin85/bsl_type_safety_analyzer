...
impl PlatformDocsCache {
    ...
    fn extract_from_hybrid_storage(&self, version: &str) -> Result<Vec<BslEntity>> {
        ...
        if !hybrid_path.exists() {
            // Если документации нет, создаем только примитивные типы
            log::info!("Platform documentation not found, creating primitive types only for version {}", version);
            let mut entities = Vec::new();
            self.add_primitive_types(&mut entities, version)?;
            
            // Сохраняем примитивные типы в кеш
        ...
        for category in &categories {
            ...
            if category_file.exists() {
                ...
                // Hybrid storage хранит данные как массив JSON объектов
                if let Ok(types_array) = serde_json::from_str::<Vec<serde_json::Value>>(&content) {
                    for type_data in types_array {
        ...
        // Добавляем примитивные типы BSL, которые отсутствуют в документации 1С
        self.add_primitive_types(&mut entities, version)?;
    ...
    fn convert_hybrid_to_entity(&self, hybrid_data: &serde_json::Value, version: &str) -> Result<BslEntity> {
        ...
        // Создаем display_name в формате "Русское (English)" как в кеше
        let display_name = if let Some(ref eng) = english_name {
            format!("{} ({})", name, eng)
        } else {
            name.to_string()
        };
        ...
        // Конвертируем методы (hybrid storage хранит как объект)
        if let Some(methods) = hybrid_data.get("methods").and_then(|v| v.as_object()) {
            for (method_name, method_data) in methods {
                if let Ok(method) = self.convert_hybrid_method(method_name, method_data) {
                    entity.interface.methods.insert(method.name.clone(), method);
                }
            }
        }
        
        // Конвертируем свойства (если есть)
        if let Some(properties) = hybrid_data.get("properties").and_then(|v| v.as_object()) {
            for (prop_name, prop_data) in properties {
                if let Ok(property) = self.convert_hybrid_property(prop_name, prop_data) {
    ...
    fn convert_hybrid_method(&self, method_name: &str, method_data: &serde_json::Value) -> Result<BslMethod> {
        use super::entity::*;
        ...
        // Конвертируем параметры из hybrid format
        if let Some(params) = method_data.get("parameters").and_then(|v| v.as_array()) {
            for param_data in params {
                if let Ok(param) = self.convert_hybrid_parameter(param_data) {
    ...
    fn convert_hybrid_property(&self, prop_name: &str, prop_data: &serde_json::Value) -> Result<BslProperty> {
        use super::entity::*;
    ...
    fn convert_hybrid_parameter(&self, param_data: &serde_json::Value) -> Result<BslParameter> {
        use super::entity::*;
        
        Ok(BslParameter {
            name: param_data.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string(),
            type_name: param_data.get("parameter_type").and_then(|v| v.as_str()).map(|s| s.to_string()),
            is_optional: !param_data.get("required").and_then(|v| v.as_bool()).unwrap_or(true),
    ...
    fn add_primitive_types(&self, entities: &mut Vec<BslEntity>, version: &str) -> Result<()> {
        ...
        if !syntax_helper_path.exists() {
            log::warn!("Syntax helper archive not found at {}, using fallback primitive types", syntax_helper_path.display());
        ...
        match self.parse_primitive_types_from_archive(&syntax_helper_path, version) {
            Ok(primitive_entities) => {
                ...
                log::info!("Successfully parsed {} primitive types from syntax helper", entities.len());
            }
            Err(e) => {
                log::warn!("Failed to parse primitive types from syntax helper: {}, using fallback", e);
    ...
    fn parse_primitive_types_from_archive(&self, archive_path: &Path, version: &str) -> Result<Vec<BslEntity>> {
        use std::io::Read;
        use zip::ZipArchive;
        ...
        let constructor_files = [
            ("objects/Global context/methods/catalog4841/String960.html", "Строка", "String"),
            ("objects/Global context/methods/catalog4841/Boolean958.html", "Булево", "Boolean"),
            ("objects/Global context/methods/catalog4841/Date961.html", "Дата", "Date"),
        ...
        for (file_path, russian_name, english_name) in &constructor_files {
            match archive.by_name(file_path) {
                Ok(mut file) => {
                    ...
                    if let Ok(entity) = self.parse_constructor_html(&content, russian_name, english_name, version) {
                        entities.push(entity);
    ...
    fn parse_constructor_html(&self, html_content: &str, russian_name: &str, english_name: &str, version: &str) -> Result<BslEntity> {
        use super::entity::*;
    ...
    fn add_missing_primitive_types(&self, entities: &mut Vec<BslEntity>, version: &str) -> Result<()> {
        use super::entity::*;
    ...
    fn add_fallback_primitive_types(&self, entities: &mut Vec<BslEntity>, version: &str) -> Result<()> {
        use super::entity::*;
