...
use crate::docs_integration::{BslFunctionInfo, BslMethodInfo, BslSyntaxDatabase};
use crate::unified_index::entity::{
    BslContext, BslEntity, BslEntityKind, BslEntitySource, BslEntityType, BslMethod, BslParameter,
    BslProperty,
};
use anyhow::Result;
...
impl SyntaxDbConverter {
    ...
    pub fn convert(&self, syntax_db: &BslSyntaxDatabase) -> Result<Vec<BslEntity>> {
                ...
                if let Some(key) = entity_key {
                    if let Some(entity) = entity_map.get_mut(&key) {
                        ...
                        entity
                            .interface
                            .methods
                            .insert(method_name.clone(), bsl_method);
                    }
                }
            } else {
                // Глобальный метод
            ...
            if let Some(object_name) = &prop_info.object_context {
                if let Some(entity) = entity_map.get_mut(object_name) {
                    let bsl_property = BslProperty {
                        ...
                        is_readonly: matches!(
                            prop_info.access_mode,
                            crate::docs_integration::AccessMode::Read
                        ),
                        is_indexed: false,
                    ...
                    entity
                        .interface
                        .properties
                        .insert(prop_name.clone(), bsl_property);
                }
            }
        }
        ...
        for (func_name, func_info) in &syntax_db.functions {
            ...
            global_entity
                .interface
                .methods
                .insert(func_name.clone(), bsl_method);
        }
    ...
    fn determine_entity_kind(name: &str) -> BslEntityKind {
        match name {
            ...
            "Число" | "Number" | "Строка" | "String" | "Булево" | "Boolean" | "Дата" | "Date" => {
...
mod tests {
    ...
    fn test_entity_kind_determination() {
        assert_eq!(
            SyntaxDbConverter::determine_entity_kind("Массив"),
            BslEntityKind::Array
        );
        assert_eq!(
            SyntaxDbConverter::determine_entity_kind("Array"),
            BslEntityKind::Array
        );
        assert_eq!(
            SyntaxDbConverter::determine_entity_kind("Строка"),
            BslEntityKind::Primitive
        );
        assert_eq!(
            SyntaxDbConverter::determine_entity_kind("Unknown"),
            BslEntityKind::System
        );
    }
    ...
    fn test_context_parsing() {
        assert_eq!(
            SyntaxDbConverter::parse_context("Client"),
            Some(BslContext::Client)
        );
        assert_eq!(
            SyntaxDbConverter::parse_context("Сервер"),
            Some(BslContext::Server)
        );
