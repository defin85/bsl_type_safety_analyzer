...
impl AnalyzeObjectsCommand {
    fn run_analysis(&self) -> Result<()> {
        ...
        for entry in index.item_index.values() {
            let stats = object_stats.entry(entry.object_name.clone()).or_default();
    ...
    fn display_results(&self, summary: &ObjectSummary) -> Result<()> {
        ...
        if !self.args.quiet {
            ...
            let stats_rows = vec![
                vec![
                    "Уникальных объектов".to_string(),
                    summary.total_unique_objects.to_string(),
                ],
                vec![
                    "Всего методов".to_string(),
                    summary.statistics.total_methods.to_string(),
                ],
                vec![
                    "Всего свойств".to_string(),
                    summary.statistics.total_properties.to_string(),
                ],
                vec![
                    "Всего функций".to_string(),
                    summary.statistics.total_functions.to_string(),
                ],
                vec![
                    "Всего операторов".to_string(),
                    summary.statistics.total_operators.to_string(),
                ],
                vec![
                    "Категорий".to_string(),
                    summary.statistics.categories_count.to_string(),
                ],
                vec![
                    "Среднее методов на объект".to_string(),
            ...
            let mut category_rows: Vec<Vec<String>> = summary
                .objects_by_category
                .iter()
                .map(|(cat, objects)| {
                    vec![get_category_display_name(cat), objects.len().to_string()]
                })
                .collect();
            ...
            writer.write_header(&format!(
                "Топ-{} объектов по количеству методов",
                self.args.top
            ))?;
            ...
            writer.write_table(&["#", "Объект", "Методов", "Свойств", "English"], top_rows)?;
...
fn determine_object_category(name: &str) -> String {
    match name {
        ...
        | "ДеревоЗначений"
        | "ФиксированныйМассив"
        | "ФиксированноеСоответствие" => "collections",
        ...
        // System
        "СистемнаяИнформация"
        | "ИнформацияОПользователе"
        | "НастройкиКлиента"
        | "РаботаСФайлами"
        | "СредстваКриптографии" => "system",

        // Date/Time
        "Дата" | "СтандартныйПериод" | "ОписаниеПериода" => {
            "datetime"
        }

        // Metadata
        "Метаданные" | "ОбъектМетаданных" | "КонфигурацияМетаданных" => {
            "metadata"
        }

        // Queries
        "Запрос" | "ПостроительЗапроса" | "РезультатЗапроса" | "ВыборкаИзРезультатаЗапроса" => {
            "query"
        }

        // Forms
        "УправляемаяФорма" | "ЭлементыФормы" | "ДанныеФормы" | "ПолеФормы" => {
            "forms"
        }

        // HTTP and Web
        "HTTPСоединение" | "HTTPЗапрос" | "HTTPОтвет" | "WSПрокси" | "WSСсылка" => {
            "web"
        }
...
fn categorize_objects(objects: &HashMap<String, ObjectStats>) -> HashMap<String, Vec<String>> {
    ...
    for name in objects.keys() {
        let category = determine_object_category(name);
        categories.entry(category).or_default().push(name.clone());
    }
