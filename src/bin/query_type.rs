...
use bsl_analyzer::unified_index::{BslLanguagePreference, UnifiedIndexBuilder};
...
impl QueryTypeCommand {
    fn run_query(&self) -> Result<()> {
        ...
        let mut builder = UnifiedIndexBuilder::new().context("Failed to create index builder")?;

        let index = builder
            .build_index(&config_path, &self.args.platform_version)
        ...
        if let Some(entity) = index.find_entity_with_preference(&self.args.name, language_pref) {
            ...
            // Show methods
            if self.args.show_methods || !self.args.show_properties {
                if self.args.show_all_methods {
                    ...
                    if entity.interface.methods.len() > 10 {
                        writer.write_line(&format!(
                            "... and {} more methods",
                            entity.interface.methods.len() - 10
                        ))?;
                    }
                }
            }
            ...
            // Show properties
            if self.args.show_properties || !self.args.show_methods {
                if self.args.show_all_methods {
                    ...
                    if entity.interface.properties.len() > 10 {
                        writer.write_line(&format!(
                            "... and {} more properties",
                            entity.interface.properties.len() - 10
                        ))?;
                    }
                }
            }
                ...
                for ts in &entity.relationships.tabular_sections {
                    ...
                    if self.args.show_all_methods {
                        for attr in &ts.attributes {
                            writer
                                .write_line(&format!("      {}: {}", attr.name, attr.type_name))?;
                        }
                    }
                }
            }
...
fn format_property(name: &str, property: &bsl_analyzer::unified_index::BslProperty) -> String {
    let readonly = if property.is_readonly {
        " [readonly]"
    } else {
        ""
    };
