...
use bsl_analyzer::bsl_parser::keywords::{
    can_be_variable, is_bsl_global_function, is_bsl_strict_keyword, BslContext,
...
impl DemoContextCommand {
    ...
    fn demonstrate_context_detection(&self) -> Vec<ContextExample> {
        let code_examples = [
            ...
            (
                "СписокЗначений",
                "    Перем Список Как СписокЗначений",
                BslContext::TypeDeclaration,
            ),
        ];
    ...
    fn display_custom_analysis(&self, demo: &AnalysisDemo) -> Result<()> {
        ...
        for analysis in &demo.real_code_tests {
            ...
            if !analysis.tokens.is_empty() {
                ...
                writer.write_table(&["Токен", "Контекст", "Переменная?", "Интерпретация"], rows)?;
            }
        }
        ...
        if self.args.stats_only {
            ...
            if !demo.ambiguity_examples.is_empty() {
                ...
                for example in &demo.ambiguity_examples {
                    ...
                    let rows: Vec<Vec<String>> = example
                        .contexts
                        .iter()
                        .map(|c| {
                            vec![
            ...
            if !demo.context_detection.is_empty() {
                ...
                writer.write_table(&["Код", "Слово", "Контекст", "Переменная?"], rows)?;
            }
    ...
    fn display_statistics(&self, writer: &mut OutputWriter, stats: &DemoStatistics) -> Result<()> {
        ...
        let rows = vec![
            vec![
                "Встроенных типов".to_string(),
                stats.builtin_types_count.to_string(),
            ],
            vec![
                "Глобальных функций".to_string(),
                stats.global_functions_count.to_string(),
            ],
            vec![
                "Системных объектов".to_string(),
                stats.system_objects_count.to_string(),
            ],
            vec![
                "Глобальных свойств".to_string(),
                stats.global_properties_count.to_string(),
            ],
            vec![
                "ИТОГО конструкций".to_string(),
                stats.total_language_constructs.to_string(),
            ],
            vec![
                "Снижение ложных срабатываний".to_string(),
                stats.false_positives_reduction.clone(),
            ],
            vec![
                "Улучшение точности".to_string(),
                stats.accuracy_improvement.clone(),
            ],
        ];
