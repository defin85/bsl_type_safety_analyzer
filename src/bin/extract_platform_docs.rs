...
use bsl_analyzer::unified_index::{PlatformDocsCache, converters::SyntaxDbConverter};
...
impl ExtractDocsCommand {
    fn extract_docs(&self) -> Result<()> {
        ...
        let rows = vec![
            ...
            vec!["Properties".to_string(), syntax_db.properties.len().to_string()],
            vec!["Functions".to_string(), syntax_db.functions.len().to_string()],
            vec!["Operators".to_string(), syntax_db.operators.len().to_string()],
    ...
    fn show_statistics(&self, writer: &mut OutputWriter, entities: &[bsl_analyzer::unified_index::BslEntity]) -> Result<()> {
