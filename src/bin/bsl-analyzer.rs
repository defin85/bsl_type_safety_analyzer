...
use anyhow::Result;
use bsl_analyzer::bsl_parser::{AnalysisConfig, BslAnalyzer};
use bsl_analyzer::cli_common::{self, CliCommand, OutputFormat, OutputWriter};
use bsl_analyzer::unified_index::{BslApplicationMode, UnifiedIndexBuilder};
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Stats { path, format } => {
            let cmd = StatsCommand { path, format };
            cli_common::run_command(cmd)?;
        }
    }
...
impl CliCommand for AnalyzeCommand {
    ...
    fn execute(&self) -> Result<()> {
        ...
        if self.path.is_file() {
            ...
            let (analyzed, errors, warnings) =
                self.analyze_directory(&self.path, &mut analyzer, &config)?;
...
impl AnalyzeCommand {
    fn create_analysis_config(&self) -> Result<AnalysisConfig> {
        let config = match self.level.to_lowercase().as_str() {
            ...
            _ => {
                anyhow::bail!(
                    "Invalid analysis level: {}. Use: syntax, semantic, dataflow, or full",
                    self.level
                );
            }
        };
        ...
        for entry in walker {
            ...
            if entry.file_type().is_file() {
                if let Some(ext) = entry.path().extension() {
                    if ext == "bsl" || ext == "os" {
                        self.analyze_file(entry.path(), analyzer, config)?;
                        files_analyzed += 1;
    ...
    fn print_diagnostics(
        ...
        let format = OutputFormat::parse_output_format(&self.format)?;
...
impl CliCommand for IndexCommand {
    ...
    fn execute(&self) -> Result<()> {
        ...
        let mut builder = UnifiedIndexBuilder::new()?.with_application_mode(app_mode);
        ...
        if self.stats {
            ...
            let rows = vec![
                vec![
                    ...
                    index.get_entity_count().to_string(),
                ],
                vec![
    ...
    fn execute(&self) -> Result<()> {
        ...
        if let Some(entity) = index.find_entity(&self.name) {
            ...
            if self.show_methods && !entity.interface.methods.is_empty() {
                ...
                for name in entity.interface.methods.keys() {
...
impl CliCommand for StatsCommand {
    ...
    fn execute(&self) -> Result<()> {
        ...
        let rows = vec![
            ...
            vec![
                "Average lines/file".to_string(),
                if file_count > 0 {
                    (line_count / file_count).to_string()
                } else {
                    "0".to_string()
                },
            ],
