...
use bsl_analyzer::docs_integration::bsl_syntax_extractor::{BslSyntaxDatabase, BslSyntaxExtractor};
...
impl ExtractHybridDocsCommand {
    fn run_extraction(&self) -> Result<()> {
        ...
        let result = ExtractionResult {
            ...
            statistics: ExtractionStatistics {
                ...
                total_items: database.objects.len()
    ...
    fn save_split_categories(&self, database: &BslSyntaxDatabase) -> Result<()> {
        ...
        if !self.args.quiet {
            cli_common::print_success(&format!("Категории сохранены в {}/", parent.display()));
        }
    ...
    fn collect_category_info(&self, database: &BslSyntaxDatabase) -> Vec<CategoryInfo> {
        ...
        // Objects
        if !database.objects.is_empty() {
            let examples: Vec<String> = database.objects.keys().take(3).cloned().collect();
        ...
        // Methods
        if !database.methods.is_empty() {
            let examples: Vec<String> = database.methods.keys().take(3).cloned().collect();
            categories.push(CategoryInfo {
        ...
        // Properties
        if !database.properties.is_empty() {
            let examples: Vec<String> = database.properties.keys().take(3).cloned().collect();
            categories.push(CategoryInfo {
        ...
        // Functions
        if !database.functions.is_empty() {
            let examples: Vec<String> = database.functions.keys().take(3).cloned().collect();
            categories.push(CategoryInfo {
        ...
        // Operators
        if !database.operators.is_empty() {
            let examples: Vec<String> = database.operators.keys().take(3).cloned().collect();
            categories.push(CategoryInfo {
    ...
    fn display_results(&self, result: &ExtractionResult) -> Result<()> {
        ...
        if !self.args.quiet {
            ...
            let stats_rows = vec![
                vec![
                    "Объектов".to_string(),
                    result.statistics.total_objects.to_string(),
                ],
                vec![
                    "Методов".to_string(),
                    result.statistics.total_methods.to_string(),
                ],
                vec![
                    "Свойств".to_string(),
                    result.statistics.total_properties.to_string(),
                ],
                vec![
                    "Функций".to_string(),
                    result.statistics.total_functions.to_string(),
                ],
                vec![
                    "Операторов".to_string(),
                    result.statistics.total_operators.to_string(),
                ],
                vec![
                    "ИТОГО элементов".to_string(),
                    result.statistics.total_items.to_string(),
                ],
            ];
            ...
            if self.args.detailed_stats {
                ...
                let perf_rows = vec![
                    ...
                    vec![
                        ...
                        format!(
                            "{:.2} MB",
                            result.statistics.archive_size_bytes as f64 / 1_048_576.0
                        ),
                    ],
                    vec![
                        ...
                        format!(
                            "{:.2} MB",
                            result.statistics.output_size_bytes as f64 / 1_048_576.0
                        ),
                    ],
                    vec![
