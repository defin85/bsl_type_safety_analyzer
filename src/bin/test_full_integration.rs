...
"#;
    ...
    match analyzer.analyze_code(test_code, "test_full_integration.bsl") {
        Ok(()) => {
            ...
            println!("   - BSL003 (Неизвестные методы): {} {}", bsl003_count, if bsl003_count > 0 { "✅" } else { "❌" });
            println!("   - BSL004 (Неверные параметры): {} {}", bsl004_count, if bsl004_count > 0 { "✅" } else { "❌" });
            println!("   - BSL007 (Необъявленные переменные): {} {}", bsl007_count, if bsl007_count > 0 { "✅" } else { "❌" });
            println!("   - BSL009 (Неинициализированные переменные): {} {}", bsl009_count, if bsl009_count > 0 { "✅" } else { "❌" });
            println!("   - BSL010 (Дублированные параметры): {} {}", bsl010_count, if bsl010_count > 0 { "✅" } else { "❌" });
            ...
            println!("\n✅ Сравнение:");
            println!("   - С UnifiedBslIndex: {} ошибок, {} предупреждений", 
                results.error_count(), results.warning_count());
            println!("   - Без UnifiedBslIndex: {} ошибок, {} предупреждений", 
                simple_results.error_count(), simple_results.warning_count());
            
            let improvement = results.error_count() + results.warning_count() - 
                             simple_results.error_count() - simple_results.warning_count();
            
            if improvement > 0 {
                println!("   🎯 Улучшение: +{} дополнительных проблем обнаружено", improvement);
            }
