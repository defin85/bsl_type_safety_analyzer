...
pub use module_generator::{ModuleContract, ModuleGenerator};
...
impl Configuration {
    ...
    pub fn load_from_directory<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let modules =
            Self::discover_bsl_modules(&path).context("Failed to discover BSL modules")?;
    ...
    pub fn find_dependents(&self, module_name: &str) -> Vec<&BslModule> {
        self.modules
            .iter()
            .filter(|m| m.imports.iter().any(|imp| imp.module_name == module_name))
            .collect()
    }
    ...
    pub fn statistics(&self) -> ConfigurationStatistics {
        ...
        let total_exports = self.modules.iter().map(|m| m.exports.len()).sum();
        let total_imports = self.modules.iter().map(|m| m.imports.len()).sum();
