...
impl ModuleGenerator {
    ...
    pub fn clean_output_directory(&mut self) -> Result<()> {
        if self.output_dir.exists() {
            self.log(
                "info",
                format!("Очищаю целевую папку: {}", self.output_dir.display()),
            );
            ...
            self.log(
                "success",
                format!("Целевая папка очищена: удалено {} файлов", deleted_files),
            );
        } else {
            self.log(
                "info",
                format!(
                    "Целевая папка не существует, будет создана: {}",
    ...
    pub fn generate(&mut self) -> Result<bool> {
        ...
        self.log(
            "info",
            format!("Выходная директория: {}", self.output_dir.display()),
        );
        ...
        self.log(
            "info",
            "Генератор контрактов модулей - это заглушка".to_string(),
        );
        self.log(
            "info",
            "Функциональность анализа метаданных покрывается другими генераторами".to_string(),
        );
        self.log(
            "success",
            "Создан информационный файл о статусе генератора".to_string(),
        );
