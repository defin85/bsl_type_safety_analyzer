...
pub use lru_cache::{LruCache, CacheStats};
...
impl CacheManager {
    ...
    pub fn load_from_file<P: AsRef<Path>>(cache_file: P) -> Result<Self> {
        let content = std::fs::read_to_string(cache_file.as_ref())
            .with_context(|| format!("Failed to read cache file: {}", cache_file.as_ref().display()))?;
        ...
        tracing::info!("Cache loaded from file: {} entries", cache_manager.memory_cache.len());
        Ok(cache_manager)
    }
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, cache_file: P) -> Result<()> {
        let cache_data = CacheData {
            entries: self.memory_cache.iter().map(|(k, v)| (k.clone(), v.clone())).collect(),
        ...
        std::fs::write(cache_file.as_ref(), content)
            .with_context(|| format!("Failed to write cache file: {}", cache_file.as_ref().display()))?;
    ...
    fn evict_entries(&mut self) {
        ...
        tracing::debug!("Cache evicted entries, current size: {} bytes", self.current_size_bytes);
    }
    
    /// Инвалидирует записи для файла
    pub fn invalidate_file(&mut self, file_path: &Path) {
        let keys_to_remove: Vec<CacheKey> = self.memory_cache
            .iter()
            .filter(|(key, _)| {
                key.file_path.as_ref().map_or(false, |p| p == file_path)
            })
            .map(|(key, _)| key.clone())
            .collect();
    ...
    pub fn garbage_collect(&mut self) {
        ...
        let expired_keys: Vec<CacheKey> = self.memory_cache
            .iter()
            .filter(|(_, value)| {
                now.duration_since(value.accessed_at)
                    .map_or(false, |duration| duration > ttl)
            })
            .map(|(key, _)| key.clone())
            .collect();
