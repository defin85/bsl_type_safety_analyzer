...
impl FileCache {
        ...
        for filename in self.metadata.keys() {
            ...
            if file_path.exists() {
                fs::remove_file(&file_path).with_context(|| {
                    format!("Failed to remove cache file: {}", file_path.display())
                })?;
            }
        }
    ...
    fn read_cache_file(&self, file_path: &Path) -> Result<FileCacheEntry> {
        ...
        if self.compression_enabled {
            ...
            bincode::deserialize(&content).context("Failed to deserialize compressed cache entry")
        } else {
            let mut content = Vec::new();
            file.read_to_end(&mut content)
                .context("Failed to read cache file")?;

            bincode::deserialize(&content).context("Failed to deserialize cache entry")
        }
    }
    ...
    fn load_metadata(&mut self) -> Result<()> {
        ...
        self.metadata = serde_json::from_str(&content).context("Failed to deserialize metadata")?;

        tracing::debug!(
            "Loaded file cache metadata: {} entries",
            self.metadata.len()
        );
        Ok(())
    }
...
mod tests {
    use super::*;
    use crate::cache::{CacheKey, CacheType, CacheValue};
