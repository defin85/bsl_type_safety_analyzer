...
impl BslParser {
    ...
    fn parse_simple_bsl_structure(
        ...
        while i < lines.len() {
            ...
            if trimmed.starts_with("Процедура ") || trimmed.starts_with("Procedure ") {
                if let Some((proc, lines_consumed)) = self.parse_simple_procedure_with_body(&lines, i, file_path) {
            ...
            if trimmed.starts_with("Функция ") || trimmed.starts_with("Function ") {
                if let Some((func, lines_consumed)) = self.parse_simple_function_with_body(&lines, i, file_path) {
    ...
    fn parse_simple_procedure(&self, line: &str, line_num: usize, file_path: &str) -> Option<ProcedureDecl> {
        ...
        let name_end = line.find('(').unwrap_or_else(|| {
            ...
            line.find(" Экспорт").or_else(|| line.find(" Export")).unwrap_or(line.len())
        });
    ...
    fn parse_simple_function(&self, line: &str, line_num: usize, file_path: &str) -> Option<FunctionDecl> {
        let export = line.contains("Экспорт") || line.contains("Export");
    ...
    fn parse_simple_variable(&self, line: &str, line_num: usize, file_path: &str) -> Option<VariableDecl> {
        let export = line.contains("Экспорт") || line.contains("Export");
        
        let name_start = if line.starts_with("Перем ") { 6 } else { 4 }; // "Var " = 4 символа
    ...
    fn parse_simple_procedure_with_body(&self, lines: &[&str], start_idx: usize, file_path: &str) -> Option<(ProcedureDecl, usize)> {
        ...
        Some((ProcedureDecl {
            ...
            directives: vec![],
            body,
            location: Location::new(file_path.to_string(), start_idx + 1, 1, 0, header_line.len()),
        }, lines_consumed))
    }
    
    /// Парсит функцию с телом
    fn parse_simple_function_with_body(&self, lines: &[&str], start_idx: usize, file_path: &str) -> Option<(FunctionDecl, usize)> {
        let header_line = lines[start_idx].trim();
        let export = header_line.contains("Экспорт") || header_line.contains("Export");
    ...
    fn parse_simple_parameters(&self, line: &str, line_num: usize, file_path: &str) -> Vec<Parameter> {
            ...
            if let Some(end) = line.find(')') {
                ...
                if !params_str.trim().is_empty() {
                    for param_str in params_str.split(',') {
                        let param_parts: Vec<&str> = param_str.trim().split_whitespace().collect();
                        if let Some(param_name) = param_parts.first() {
                            if !param_name.is_empty() {
                                params.push(Parameter {
                                    ...
                                    location: Location::new(file_path.to_string(), line_num, 1, 0, param_str.len()),
                                });
                            }
                        }
                    }
    ...
    fn parse_simple_statement(&self, line: &str, line_num: usize, file_path: &str) -> Option<Statement> {
        ...
        // Парсим присваивание
        if trimmed.contains(" = ") {
            return self.parse_simple_assignment(trimmed, line_num, file_path).map(Statement::Assignment);
        }
        
        // Парсим условный оператор
        if trimmed.starts_with("Если ") || trimmed.starts_with("If ") {
            return self.parse_simple_if(trimmed, line_num, file_path).map(Statement::If);
        }
        
        // Парсим возврат
        if trimmed.starts_with("Возврат ") || trimmed.starts_with("Return ") {
            return self.parse_simple_return(trimmed, line_num, file_path).map(Statement::Return);
        }
    ...
    /// Парсит присваивание
    fn parse_simple_assignment(&self, line: &str, line_num: usize, file_path: &str) -> Option<Assignment> {
    ...
    fn parse_simple_if(&self, line: &str, line_num: usize, file_path: &str) -> Option<IfStatement> {
        ...
        if let Some(condition) = self.parse_simple_expression(condition_str, line_num, file_path) {
            Some(IfStatement {
                condition,
    ...
    /// Парсит возврат
    fn parse_simple_return(&self, line: &str, line_num: usize, file_path: &str) -> Option<ReturnStatement> {
    ...
    fn parse_simple_expression(&self, expr: &str, line_num: usize, file_path: &str) -> Option<Expression> {
        ...
        if trimmed.parse::<f64>().is_ok() {
            return Some(Expression::Literal(Literal::Number(trimmed.parse().unwrap_or(0.0))));
        }
        ...
        if trimmed.chars().all(|c| c.is_alphanumeric() || c == '_' || "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя".contains(c)) {
    ...
    /// Парсит вызов метода
    fn parse_simple_method_call(&self, expr: &str, line_num: usize, file_path: &str) -> Option<Expression> {
    ...
    fn parse_simple_new_expression(&self, expr: &str, line_num: usize, file_path: &str) -> Option<Expression> {
        let type_start = if expr.starts_with("Новый ") { 6 } else { 4 };
        let paren_pos = expr.find('(')?;
    ...
    fn parse_simple_function_call(&self, expr: &str, line_num: usize, file_path: &str) -> Option<Expression> {
        let paren_pos = expr.find('(')?;
        let function_name = expr[..paren_pos].trim();
        
        // Проверяем, что это допустимая функция
        if !function_name.chars().all(|c| c.is_alphanumeric() || c == '_' || "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯабвгдеёжзийклмнопрстуфхцчшщъыьэюя".contains(c)) {
            return None;
        }
        ...
        if !args_str.trim().is_empty() {
            for arg_str in args_str.split(',') {
                if let Some(arg_expr) = self.parse_simple_expression(arg_str.trim(), line_num, file_path) {
                    args.push(arg_expr);
                }
            }
        }
    ...
    fn parse_simple_property_access(&self, expr: &str, line_num: usize, file_path: &str) -> Option<Expression> {
        let dot_pos = expr.find('.')?;
        let object_str = &expr[..dot_pos];
