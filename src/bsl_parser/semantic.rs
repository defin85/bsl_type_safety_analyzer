...
impl SemanticAnalyzer {
            ...
            if method_call.method.contains('.') && object_type.is_some() {
                ...
                if parts.len() == 2 {
                    ...
                    if let Some(manager_info) = self.parse_manager_type(object_type_name) {
                        ...
                        println!("🔧 ИСПРАВЛЕНО: Поиск конкретного объекта {}: {}", manager_info.kind, concrete_type);
                        ...
                        if let Some(entity) = index.find_entity(&concrete_type) {
                            ...
                            println!("🔍 DEBUG: Найден объект {}, методов: {}", concrete_type, all_methods.len());
                            ...
                            if method_found {
                                println!("✅ ИСПРАВЛЕНО: Метод {} найден в конкретном объекте {}", method_name, concrete_type);
                                return Ok(()); // Метод найден - всё в порядке
                            } else {
                                println!("❌ ИСПРАВЛЕНО: Метод {} НЕ найден в конкретном объекте {}", method_name, concrete_type);
                                ...
                                self.diagnostics.push(
                                    ...
                                    .with_expected(&format!("доступные методы объекта {}", concrete_type))
                                );
                                return Ok(()); // Обработали ошибку
                            }
                        } else {
                            println!("❌ ИСПРАВЛЕНО: Конкретный объект {} не найден в конфигурации", concrete_type);
    ...
    fn parse_manager_type(&self, type_name: &str) -> Option<ManagerInfo> {
        ...
        let manager_patterns = [
            ...
            ("InformationRegistersManager", "РегистрСведенийМенеджер", "Регистр сведений"),
            ("МенеджерРегистровСведений", "РегистрСведенийМенеджер", "Регистр сведений"),
            ("AccumulationRegistersManager", "РегистрНакопленияМенеджер", "Регистр накопления"),
            ("МенеджерРегистровНакопления", "РегистрНакопленияМенеджер", "Регистр накопления"),
