...
struct ManagerInfo {
    ...
    kind: String,
    /// Вид объекта для сообщений (родительный падеж, например, "справочника")
    kind_genitive: String,
}
...
impl SemanticAnalyzer {
            ...
            Expression::Identifier(name) => {
                ...
                if !keywords::is_bsl_reserved_word(name) {
                    ...
                    if self.config.check_undeclared_variables {
                        if self.current_scope.get_variable(name).is_none() {
            ...
            Expression::MethodCall(method_call) => {
                ...
                if method_call.method == "=" && method_call.args.len() == 1 {
                    ...
                    if let Expression::Identifier(var_name) = &*method_call.object {
                        self.handle_assignment_inference(var_name, &method_call.args[0], method_call.location.clone())?;
                    }
                } else {
    ...
    fn is_part_of_complex_expression(&self, name: &str) -> bool {
        ...
        complex_patterns.iter().any(|pattern| pattern.contains(name))
    }
    ...
    fn validate_method_call(&mut self, method_call: &MethodCall) -> Result<()> {
        if let Some(index) = &self.index {
            ...
            println!("🔍 DEBUG MethodCall: object={:?}, method={}, object_type={:?}", 
            ...
            if method_call.method.contains('.') && object_type.is_some() {
                ...
                println!("🔧 WORKAROUND: Обрабатываем составной метод: {} для {}", method_call.method, object_type_name);
                ...
                if parts.len() == 2 {
                    ...
                    // Универсальная обработка менеджеров 1С через паттерн
                    if let Some(manager_info) = self.parse_manager_type(object_type_name) {
                        // ООП-подход: используем шаблонный тип для всех объектов данного вида
                        let template_type = format!("{}.{}", manager_info.base_type, "<Имя справочника>");
                        println!("🔧 WORKAROUND: Поиск шаблонного типа {}: {}", manager_info.kind, template_type);
                        
                        // Проверяем существование метода в шаблонном типе (как в ООП наследовании)
                        if let Some(entity) = index.find_entity(&template_type) {
                            ...
                            if method_found {
                                println!("✅ WORKAROUND: Метод {} найден в шаблонном типе {}", method_name, template_type);
                                return Ok(()); // Метод найден - всё в порядке
                            } else {
                                println!("❌ WORKAROUND: Метод {} НЕ найден в шаблонном типе {}", method_name, template_type);
                                ...
                                self.diagnostics.push(
                                    ...
                                    .with_expected(&format!("доступные методы {}", manager_info.kind_genitive))
                                );
                                return Ok(()); // Обработали ошибку
                            }
                        } else {
                            println!("❌ WORKAROUND: Шаблонный тип {} {} не найден", manager_info.kind, template_type);
                            ...
                            self.diagnostics.push(
                                Diagnostic::new(
                                    ...
                                    format!("{} '{}' не найден в конфигурации", manager_info.kind, property_name),
                                )
                            );
                            return Ok(()); // Обработали ошибку
                        }
            ...
            if let Some(type_name) = object_type {
                ...
                if let Some(entity) = index.find_entity(&type_name) {
                    println!("🔍 DEBUG найден entity: qualified_name='{}', display_name='{}'", 
                        entity.qualified_name, entity.display_name);
                    
                    // Получаем все методы (включая унаследованные)
                    let all_methods = index.get_all_methods(&entity.qualified_name);
                    println!("🔍 DEBUG методы для {}: {:?}", entity.qualified_name, 
    ...
    fn validate_function_call(&mut self, function_call: &FunctionCall) -> Result<()> {
        ...
        self.diagnostics.push(
            Diagnostic::new(
                ...
                codes::UNKNOWN_METHOD,
                format!("Функция '{}' не найдена ни в глобальном контексте, ни в текущем модуле", function_call.name),
            )
    ...
    fn handle_assignment_inference(&mut self, var_name: &str, value_expr: &Expression, location: Location) -> Result<()> {
        // Не проверяем ключевые слова BSL
        if !keywords::is_bsl_reserved_word(var_name) {
            ...
            if let Some(inferred_type) = self.infer_expression_type(value_expr) {
                println!("🔍 Вывод типа: {} = {} (тип: {})", var_name, 
                    match value_expr {
                        Expression::New(new_expr) => format!("Новый {}()", new_expr.type_name),
                        Expression::FunctionCall(func) => format!("{}()", func.name),
                        Expression::Literal(_) => "литерал".to_string(),
    ...
    fn parse_manager_type(&self, type_name: &str) -> Option<ManagerInfo> {
        ...
        let manager_patterns = [
            // (паттерн для поиска, базовый тип, вид именительный, вид родительный)
            ("Справочники", "СправочникМенеджер", "Справочник", "справочника"),           // Глобальный алиас
            ("CatalogsManager", "СправочникМенеджер", "Справочник", "справочника"),
            ("СправочникиМенеджер", "СправочникМенеджер", "Справочник", "справочника"),
            ("DocumentsManager", "ДокументМенеджер", "Документ", "документа"),
            ("МенеджерДокументов", "ДокументМенеджер", "Документ", "документа"),
            ("InformationRegistersManager", "РегистрСведенийМенеджер", "Регистр сведений", "регистра сведений"),
            ("МенеджерРегистровСведений", "РегистрСведенийМенеджер", "Регистр сведений", "регистра сведений"),
            ("AccumulationRegistersManager", "РегистрНакопленияМенеджер", "Регистр накопления", "регистра накопления"),
            ("МенеджерРегистровНакопления", "РегистрНакопленияМенеджер", "Регистр накопления", "регистра накопления"),
            ("DataProcessorsManager", "ОбработкаМенеджер", "Обработка", "обработки"),
            ("МенеджерОбработок", "ОбработкаМенеджер", "Обработка", "обработки"),
            ("ReportsManager", "ОтчетМенеджер", "Отчет", "отчета"),
            ("МенеджерОтчетов", "ОтчетМенеджер", "Отчет", "отчета"),
            ("ConstantsManager", "КонстантаМенеджер", "Константа", "константы"),
            ("МенеджерКонстант", "КонстантаМенеджер", "Константа", "константы"),
        ];
        
        for (pattern, base_type, kind, kind_genitive) in &manager_patterns {
            if type_name.contains(pattern) {
                return Some(ManagerInfo {
                    base_type: base_type.to_string(),
                    kind: kind.to_string(),
                    kind_genitive: kind_genitive.to_string(),
                });
            }
        }
    ...
    fn infer_expression_type(&self, expr: &Expression) -> Option<String> {
        match expr {
            ...
            Expression::PropertyAccess(prop_access) => {
                ...
                println!("🔍 DEBUG PropertyAccess: object={:?}, property={}", 
                ...
                if let Some(index) = &self.index {
                    if let Some(object_type) = self.infer_expression_type(&prop_access.object) {
                        ...
                        if let Some(manager_info) = self.parse_manager_type(&object_type) {
                            ...
                            let target_type = format!("{}.{}", manager_info.base_type, prop_access.property);
                            println!("🔍 DEBUG PropertyAccess manager: {} -> {}", object_type, target_type);
                            return Some(target_type);
                        }
                        
                        // Стандартная обработка свойств
                        if let Some(entity) = index.find_entity(&object_type) {
                            if let Some(property) = entity.interface.properties.get(&prop_access.property) {
