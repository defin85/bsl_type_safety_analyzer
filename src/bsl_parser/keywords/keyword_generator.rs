...
impl GeneratedBslKeywords {
        ...
        for (line_num, line) in reader.lines().enumerate() {
            ...
            match parse_platform_entity(&line) {
                ...
                Err(e) => {
                    ...
                    eprintln!("Warning: Failed to parse entity at line {}: {}", line_num + 1, e);
                }
            }
        }
    ...
    fn validate(&self) -> Result<()> {
        if self.builtin_types.is_empty() {
            return Err(anyhow::anyhow!("No builtin types loaded from platform cache"));
        }
        
        if self.global_functions.is_empty() {
            return Err(anyhow::anyhow!("No global functions loaded from platform cache"));
        }
        ...
        for required_type in &required_types {
            let found = self.builtin_types.contains(*required_type) ||
                       self.builtin_types.iter().any(|t| t.starts_with(required_type) && t.contains('('));
            if !found {
                eprintln!("Warning: Required type '{}' not found in platform cache", required_type);
            }
        }
        
        println!("Loaded {} builtin types, {} global functions, {} system objects", 
    ...
    pub fn can_be_variable(&self, word: &str, context: BslContext) -> bool {
        ...
        match context {
            ...
            BslContext::TypeDeclaration => {
                ...
                self.is_builtin_type(word) || self.is_system_object(word) || !is_bsl_strict_keyword(word)
            }
...
fn parse_platform_entity(json_line: &str) -> Result<PlatformEntityInfo> {
    ...
    let qualified_name = value["qualified_name"]
        .as_str()
        .unwrap_or("")
        .to_string();
    ...
    let english_name = value["english_name"]
        .as_str()
        .map(|s| s.to_string());
...
fn is_system_object(qualified_name: &str) -> bool {
    ...
    system_patterns.iter().any(|pattern| qualified_name.starts_with(pattern))
}
...
pub static GENERATED_BSL_KEYWORDS: Lazy<GeneratedBslKeywords> = Lazy::new(|| {
    ...
    GeneratedBslKeywords::load_from_platform_cache("8.3.25")
        .unwrap_or_else(|e| {
            eprintln!("Warning: Failed to load platform keywords: {}. Using fallback.", e);
...
mod tests {
    ...
    fn test_extract_method_name() {
        assert_eq!(extract_method_name("Массив.Добавить"), Some("Добавить".to_string()));
        assert_eq!(extract_method_name("Сообщить"), Some("Сообщить".to_string()));
        assert_eq!(extract_method_name("ТаблицаЗначений.Вставить"), Some("Вставить".to_string()));
    }
    
    #[test]
    fn test_extract_root_object_name() {
        assert_eq!(extract_root_object_name("Справочники.Номенклатура"), Some("Справочники".to_string()));
        assert_eq!(extract_root_object_name("Метаданные"), Some("Метаданные".to_string()));
        assert_eq!(extract_root_object_name("Документы.ЗаказНаряды"), Some("Документы".to_string()));
    }
    ...
    fn test_platform_cache_loading() {
        ...
        if get_platform_cache_path("8.3.25").unwrap().exists() {
            ...
            println!("Platform cache test passed: {} types, {} functions loaded", 
