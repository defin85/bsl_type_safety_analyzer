...
mod tests {
    ...
    fn test_empty_code_parsing() {
        ...
        assert!(
            ...
            "У пустого модуля не должно быть дочерних узлов"
        );
    }
    ...
    fn test_variable_declaration_parsing() {
        ...
        let ast = result.unwrap();
        assert_eq!(
            count_nodes_of_type(&ast, AstNodeType::VariableDeclaration),
            1
        );
        ...
        assert!(
            var_decl.is_some(),
            "Должно быть найдено объявление переменной"
        );

        if let Some(var_node) = var_decl {
            assert!(
                var_node.value.is_some(),
                "Объявление переменной должно иметь имя"
            );
    ...
    fn test_multiple_variable_declarations() {
        ...
        assert!(
            result.is_ok(),
            "Множественные объявления переменных должны парситься"
        );
        ...
        assert!(
            var_count >= 3,
            "Должно быть найдено минимум 3 объявления переменных, найдено: {}",
            var_count
        );
    }
    ...
    fn test_procedure_with_parameters() {
        ...
        if let Some(proc_node) = proc_decl {
            ...
            assert!(
                params >= 3,
                "Процедура должна иметь 3 параметра, найдено: {}",
                params
            );
        }
    }
    ...
    fn test_if_statement_parsing() {
        ...
        assert!(
            if_stmt.is_some(),
            "Должна быть найдена условная конструкция"
        );
    }
    ...
    fn test_try_catch_parsing() {
        ...
        assert!(
            result.is_ok(),
            "Конструкция Попытка-Исключение должна парситься"
        );
    ...
    fn test_assignment_parsing() {
        ...
        assert!(
            assignments >= 3,
            "Должно быть найдено минимум 3 присваивания, найдено: {}",
            assignments
        );
    }
    ...
    fn test_method_call_parsing() {
        ...
        assert!(
            calls >= 2,
            "Должно быть найдено минимум 2 вызова, найдено: {}",
            calls
        );
    }
    ...
    fn test_complex_procedure_parsing() {
        ...
        println!(
            "Procedure count: {}",
            count_nodes_of_type(&ast, AstNodeType::Procedure)
        );
        println!(
        ...
        println!(
            "IfStatement count: {}",
            count_nodes_of_type(&ast, AstNodeType::IfStatement)
        );
        println!(
            "ForLoop count: {}",
            count_nodes_of_type(&ast, AstNodeType::ForLoop)
        );
        println!(
            "TryStatement count: {}",
            count_nodes_of_type(&ast, AstNodeType::TryStatement)
        );
        println!(
            "Assignment count: {}",
            count_nodes_of_type(&ast, AstNodeType::Assignment)
        );
        println!(
            "CallExpression count: {}",
            count_nodes_of_type(&ast, AstNodeType::CallExpression)
        );
    ...
    fn test_nested_conditions() {
        ...
        assert!(
            if_count >= 3,
            "Должно быть найдено минимум 3 условные конструкции, найдено: {}",
            if_count
        );
    }
    ...
    fn test_syntax_error_handling() {
        ...
        assert!(
            result.is_ok(),
            "Анализатор должен обрабатывать синтаксические ошибки gracefully"
        );
    }
}
