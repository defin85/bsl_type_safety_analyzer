...
impl SyntaxAnalyzer {
    ...
    pub fn parse(&mut self, code: &str) -> Result<AstNode> {
        ...
        let tokens = lexer.tokenize(code).map_err(|e| anyhow!("Tokenization error: {}", e))?;
    ...
    fn parse_procedure(&mut self) -> Result<Option<AstNode>> {
        ...
        if let Some(proc_name) = self.consume_identifier() {
            let mut procedure = AstNode::procedure(
                Span::new(start_pos, self.current_position()),
                proc_name
            );
    ...
    fn parse_function(&mut self) -> Result<Option<AstNode>> {
        ...
        if let Some(func_name) = self.consume_identifier() {
            let mut function = AstNode::function(
                Span::new(start_pos, self.current_position()),
                func_name
            );
    ...
    fn parse_try_statement(&mut self) -> Result<Option<AstNode>> {
        ...
        let try_block = self.parse_block_until_keywords(&[TokenType::Исключение, TokenType::КонецПопытки])?;
            ...
            if self.match_token(&TokenType::Equal) {
                ...
                if let Some(method_name) = self.consume_identifier() {
                    if self.match_token(&TokenType::LeftParen) {
                        ...
                        Ok(Some(AstNode::identifier(Span::new(start_pos, self.current_position()), identifier)))
                    }
                } else {
                    Ok(Some(AstNode::identifier(Span::new(start_pos, self.current_position()), identifier)))
                }
    ...
    fn is_statement_terminator(&self) -> bool {
        ...
        matches!(self.tokens[self.current_index].token_type, 
                 TokenType::Перем | TokenType::Процедура | TokenType::Функция |
                 TokenType::If | TokenType::For | TokenType::While |
                 TokenType::КонецПроцедуры | TokenType::КонецФункции |
                 TokenType::EndIf | TokenType::EndDo)
    }
}
