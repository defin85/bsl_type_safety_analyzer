...
impl IncrementalParser {
    ...
    pub fn apply_edit(&mut self, edit: TextEdit) -> Result<Arc<AstNode>> {
        info!(
            "Applying incremental edit at {}:{}",
            edit.start.line, edit.start.column
        );
    ...
    fn node_intersects_range(&self, node: &AstNode, range: &Span) -> bool {
        ...
        !(node_span.end.offset < range.start.offset || node_span.start.offset > range.end.offset)
    }

    fn rebuild_affected_nodes(
        &mut self,
            ...
            if !line.trim().is_empty() {
                let line_span = Span::new(
                    ...
                    Position::new(
                        range.start.line + i,
                        line.len(),
                        range.start.offset + i * 50 + line.len(),
                    ),
                );

                let stmt_node =
                    AstNode::with_value(AstNodeType::Assignment, line_span, line.to_string());
    ...
    fn cache_node_recursive(
        ...
        let cache_key = format!(
            "{}:{}:{}",
            node.span.start.line, node.span.start.column, node.node_type
        );
