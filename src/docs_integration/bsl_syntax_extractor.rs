...
use super::hbk_parser_full::{HbkArchiveParser, LinkInfo, HtmlContent};
use super::chunked_writer::{StreamingChunkedWriter, ChunkSettings, SyntaxItem, ItemMetadata, LinkInfo as ChunkLinkInfo};
...
impl BslSyntaxExtractor {
    ...
    /// Создает элемент синтаксиса из информации
    fn create_syntax_item(&self, syntax_info: SyntaxInfo, index: usize) -> Result<SyntaxItem> {
        use crate::docs_integration::chunked_writer::{SyntaxItem, ItemMetadata};
        
        // Определяем категорию элемента
        let category = self.determine_item_category(&syntax_info);
        
        // Создаем ID элемента
        let id = format!("{}_{}", category, index);
        
        // Создаем метаданные
        let metadata = ItemMetadata {
            filename: syntax_info.filename.clone(),
            syntax: syntax_info.syntax.clone(),
            syntax_variants: syntax_info.syntax_variants.iter().map(|v| v.syntax.clone()).collect(),
            parameters: syntax_info.parameters.iter().map(|p| p.name.clone()).collect(),
            parameters_by_variant: syntax_info.parameters_by_variant.iter().map(|(k, v)| {
                (k.clone(), v.iter().map(|p| p.name.clone()).collect())
            }).collect(),
            return_value: syntax_info.return_value.clone(),
            example: syntax_info.example.clone(),
            links: syntax_info.links.iter().map(|link| {
                crate::docs_integration::chunked_writer::LinkInfo {
                    text: link.text.clone(),
                    href: link.href.clone(),
                }
            }).collect(),
            collection_elements: syntax_info.collection_elements
                .as_ref()
                .map(|ce| {
                    let mut map = std::collections::HashMap::new();
                    // Создаем описание элементов коллекции на основе доступной информации
                    if let Some(desc) = &ce.description {
                        map.insert("element".to_string(), desc.clone());
                    }
                    if let Some(usage) = &ce.usage {
                        map.insert("usage".to_string(), usage.clone());
                    }
                    if let Some(element_type) = &ce.element_type {
                        map.insert("type".to_string(), element_type.clone());
                    }
                    map
                })
                .unwrap_or_default(),
            methods: syntax_info.methods.iter().map(|m| m.name.clone()).collect(),
            availability: syntax_info.availability.clone(),
            version: syntax_info.version.clone(),
        };
        
        Ok(SyntaxItem {
            id,
            title: syntax_info.title.clone(),
            category,
            content: syntax_info.description.clone(),
            metadata,
        })
    }

    /// Определяет категорию элемента по его типу
    fn determine_item_category(&self, syntax_info: &SyntaxInfo) -> String {
        let title = &syntax_info.title;
        let category = &syntax_info.category;
        
        // Проверяем содержимое заголовка
        if title.contains('.') && !title.contains("Формирование") {
            // Это метод или свойство объекта
            if syntax_info.return_value.is_empty() && !syntax_info.syntax.contains("возвращает") {
            ...
            "operators".to_string()
        } else if category == "function" || syntax_info.syntax.contains("(") || !syntax_info.return_value.is_empty() {
            "functions".to_string()
        } else {
            "objects".to_string()
        }
    ...
    /// Извлекает синтаксис напрямую в гибридный формат
    pub fn extract_to_hybrid_storage<P: AsRef<Path>>(
        &mut self,
        output_dir: P,
        ...
        tracing::info!("Starting hybrid extraction to: {}", output_dir.as_ref().display());
        ...
        // Получаем список HTML файлов
        let html_files = self.hbk_parser.list_contents();
        ...
        for (index, file_path) in html_files.iter().take(process_count).enumerate() {
            ...
            // Извлекаем содержимое файла
            if let Some(content) = self.hbk_parser.extract_file_content(file_path) {
                // Парсим синтаксис из HTML
                if let Ok(syntax_info) = self.extract_syntax_info(&content, file_path) {
                    // Создаем элемент синтаксиса
                    let item = self.create_syntax_item(syntax_info, index)?;
        ...
        // Завершаем обработку и записываем файлы
        storage.finalize()?;
        
        tracing::info!("Hybrid extraction completed: {} objects, {} methods, {} properties, {} functions, {} operators", 
            objects_count, methods_count, properties_count, functions_count, operators_count);
        
        Ok(())
    }

    /// Извлекает синтаксис и экспортирует в разбитые файлы (устаревший метод)
    pub fn extract_and_export_chunked<P: AsRef<Path>>(
        &mut self,
        output_dir: P,
        max_files: Option<usize>
    ) -> Result<()> {
        tracing::info!("Starting chunked extraction to: {}", output_dir.as_ref().display());
        ...
        let mut writer = StreamingChunkedWriter::new(output_dir, settings);
        ...
        let all_files = self.hbk_parser.list_contents();
        let html_files: Vec<String> = all_files.into_iter()
            .filter(|f| f.ends_with(".html") || f.ends_with(".htm"))
            .collect();
        let files_to_process = max_files.unwrap_or(html_files.len()).min(html_files.len());
        ...
        for (idx, filename) in html_files.iter().take(files_to_process).enumerate() {
            ...
            match self.hbk_parser.extract_file_content(filename) {
                Some(content) => {
                    if let Ok(html_content) = self.hbk_parser.parse_html_content(&content) {
                        // Конвертируем HtmlContent в SyntaxInfo
                        if let Ok(syntax_info) = self.convert_html_to_syntax_info(&html_content, filename) {
                            // Конвертируем в SyntaxItem для writer'а
                            if let Some(item) = self.convert_to_syntax_item(&syntax_info, &mut objects_count, &mut methods_count, &mut properties_count, &mut functions_count, &mut operators_count) {
        ...
        tracing::info!(
            "Chunked export completed: {} objects, {} methods, {} properties, {} functions, {} operators",
            objects_count, methods_count, properties_count, functions_count, operators_count
        );
    ...
    /// Конвертирует HtmlContent в SyntaxInfo
    fn convert_html_to_syntax_info(&self, html_content: &HtmlContent, filename: &str) -> Result<SyntaxInfo> {
        // Определяем категорию по заголовку и содержимому
        let category = if self.is_function_syntax(&html_content.title) {
            "function"
        } else if self.is_operator_syntax(&html_content.title) {
            "operator"
        } else if self.is_property_syntax(&html_content.title) {
            "property"
        } else if self.is_method_syntax(&html_content.title) {
            "method"
        } else {
            "object"
        };
        
        // Парсим варианты синтаксиса из строки syntax
        let syntax_variants = if !html_content.syntax.is_empty() {
            vec![SyntaxVariant {
                variant_name: "default".to_string(),
                syntax: html_content.syntax.clone(),
            }]
        } else {
            vec![]
        };
        
        // Конвертируем поля в методы (если это объект)
        let methods = html_content.fields.iter()
            .map(|f| MethodInfo {
                name: f.name.clone(),
        ...
        // Извлекаем доступность из описания
        let availability = self.extract_availability_from_description(&html_content.description);
        
        // Извлекаем версию из описания
        let version = self.extract_version_from_description(&html_content.description);
        
        Ok(SyntaxInfo {
            filename: filename.to_string(),
            title: html_content.title.clone(),
            syntax: html_content.syntax.clone(),
            syntax_variants,
            description: html_content.description.clone(),
            parameters: vec![], // TODO: извлечь из синтаксиса
            parameters_by_variant: HashMap::new(),
            return_value: String::new(), // TODO: извлечь из описания
            example: html_content.example.clone(),
            category: category.to_string(),
            links: html_content.links.clone(),
            availability,
            version,
            methods,
            collection_elements: None,
    ...
    /// Извлекает доступность из описания
    fn extract_availability_from_description(&self, description: &str) -> Vec<String> {
        let mut availability = Vec::new();
        
        if description.contains("Тонкий клиент") {
            availability.push("Тонкий клиент".to_string());
        }
        if description.contains("Толстый клиент") {
            availability.push("Толстый клиент".to_string());
        }
        if description.contains("Сервер") || description.contains("сервер") {
            availability.push("Сервер".to_string());
        }
        if description.contains("Внешнее соединение") {
            availability.push("Внешнее соединение".to_string());
        }
        if description.contains("Мобильное приложение") {
            availability.push("Мобильное приложение".to_string());
        }
        
        availability
    }
    
    /// Извлекает версию из описания
    fn extract_version_from_description(&self, description: &str) -> String {
        // Ищем паттерны версий типа "8.3.x"
        let version_pattern = Regex::new(r"(\d+\.\d+(?:\.\d+)?)").unwrap();
        
        if let Some(captures) = version_pattern.captures(description) {
    ...
    /// Конвертирует SyntaxInfo в SyntaxItem для chunked writer
    fn convert_to_syntax_item(
        &self,
        syntax_info: &SyntaxInfo,
        objects_count: &mut usize,
        methods_count: &mut usize,
        properties_count: &mut usize,
        functions_count: &mut usize,
        operators_count: &mut usize,
    ) -> Option<SyntaxItem> {
        // Определяем категорию
        let (category, counter) = match syntax_info.category.as_str() {
            "object" => ("objects", objects_count),
            "method" => ("methods", methods_count),
            "property" => ("properties", properties_count),
            "function" => ("functions", functions_count),
            "operator" => ("operators", operators_count),
            _ => return None,
        };
        
        // Генерируем ID
        let id = format!("{}_{}", category, counter);
        *counter += 1;
        
        // Конвертируем links
        let links: Vec<ChunkLinkInfo> = syntax_info.links.iter()
            .map(|link| ChunkLinkInfo {
                text: link.text.clone(),
                href: link.href.clone(),
            })
            .collect();
        ...
        let metadata = ItemMetadata {
            filename: syntax_info.filename.clone(),
            syntax: syntax_info.syntax.clone(),
            syntax_variants: syntax_info.syntax_variants.iter()
                .map(|v| v.syntax.clone())
                .collect(),
            parameters: syntax_info.parameters.iter()
                .map(|p| p.name.clone())
                .collect(),
            parameters_by_variant: HashMap::new(), // TODO: заполнить правильно
            return_value: syntax_info.return_value.clone(),
            example: syntax_info.example.clone(),
            links,
            collection_elements: HashMap::new(), // TODO: заполнить из collection_elements
            methods: syntax_info.methods.iter()
                .map(|m| m.name.clone())
                .collect(),
            availability: syntax_info.availability.clone(),
            version: syntax_info.version.clone(),
