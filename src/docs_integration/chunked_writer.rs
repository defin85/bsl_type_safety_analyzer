...
impl ChunkedSyntaxWriter {
    ...
    pub fn write_all(&mut self) -> Result<()> {
        tracing::info!(
            "Writing chunked syntax data to: {}",
    ...
    fn write_category(&self, category: &str, items: &[SyntaxItem]) -> Result<CategoryInfo> {
        ...
        let index_path = self
            .output_dir
            .join(category)
            .join(format!("{}_index.json", category));
    ...
    pub fn write_enhanced_main_index(
        &self,
        categories: HashMap<String, CategoryInfo>,
    ) -> Result<()> {
        ...
        let mut top_objects: Vec<ObjectStat> = object_stats
            .iter()
        ...
        tracing::info!(
            "Enhanced main index written with {} total items in {} files",
            total_items,
            total_files
        );
...
impl StreamingChunkedWriter {
    ...
    pub fn add_item(&mut self, item: SyntaxItem) {
        ...
        let current_items = self.current_chunks.entry(category.clone()).or_default();
            ...
            if filepath.exists() {
                ...
                if let Ok(chunk_file) = serde_json::from_str::<ChunkFile>(&content) {
                    chunks.push(ChunkInfo {
                        ...
                        first_item_id: chunk_file
                            .items
                            .first()
                            .map(|i| i.id.clone())
                            .unwrap_or_default(),
                        last_item_id: chunk_file
                            .items
                            .last()
                            .map(|i| i.id.clone())
                            .unwrap_or_default(),
