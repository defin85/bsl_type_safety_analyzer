...
impl ChunkedDocsLoader {
    ...
    pub fn load_index(&mut self) -> Result<()> {
        ...
        let content = fs::read_to_string(&index_path).context("Failed to read main_index.json")?;

        let main_index: MainIndex =
            serde_json::from_str(&content).context("Failed to parse main_index.json")?;
    ...
    fn build_item_index(
        &self,
        categories: &HashMap<String, CategoryInfo>,
    ) -> Result<HashMap<String, ItemLocation>> {
    ...
    pub fn get_item(&self, item_id: &str) -> Result<Option<SyntaxItem>> {
        let cache = self.index_cache.as_ref().context("Index not loaded")?;

        if let Some(location) = cache.item_index.get(item_id) {
            let file_path = self.docs_dir.join(&location.category).join(&location.file);

            let content = fs::read_to_string(&file_path)?;
            let chunk_data: ChunkFile = serde_json::from_str(&content)?;

            Ok(chunk_data.items.into_iter().find(|item| item.id == item_id))
        } else {
            Ok(None)
        }
    }
                    ...
                    if let Some(file_name) = file_value.as_str() {
                        ...
                        if let Ok(chunk_content) = fs::read_to_string(&file_path) {
                            if let Ok(chunk_data) =
                                serde_json::from_str::<ChunkFile>(&chunk_content)
                            {
                                for item in chunk_data.items {
                                    let object_name = extract_object_name(&item.title);
...
mod tests {
    ...
    fn test_extract_object_name() {
        ...
        assert_eq!(extract_object_name("Массив"), "Массив");
    }
}
