...
use super::{RuleId, RuleStats, RulesConfig};
...
impl RulesEngine {
    ...
    pub fn apply_rules(&mut self, results: &AnalysisResults) -> Result<AnalysisResults> {
        ...
        if let Some(duration) = results.analysis_duration() {
            new_results.add_metric(
                "original_analysis_time".to_string(),
                format!("{:?}", duration),
            );
        }
        ...
        new_results.add_metric(
            "rules_processing_time_us".to_string(),
    ...
    pub fn validate_rules(&self, results: &AnalysisResults) -> Result<Vec<String>> {
        ...
        for rule_id in self.config.rules.keys() {
            let mut found = false;
    ...
    pub fn get_summary(&self) -> RulesSummary {
        ...
        let total_applications: u64 = self
            .stats_cache
            .values()
