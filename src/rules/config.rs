...
impl Default for RulesConfig {
    fn default() -> Self {
        ...
        for (rule_id, description, severity) in builtin_rules {
            rules.insert(rule_id.to_string(), RuleConfig {
                enabled: true,
                severity,
        ...
        // Strict profile
        profiles.insert("strict".to_string(), RuleProfile {
        ...
        // Lenient profile
        profiles.insert("lenient".to_string(), RuleProfile {
...
impl RulesConfig {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        let content = std::fs::read_to_string(&path)
            .with_context(|| format!("Failed to read rules config from {}", path.as_ref().display()))?;
    ...
    pub fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        let content = toml::to_string_pretty(self)
            .context("Failed to serialize rules config to TOML")?;
        
        std::fs::write(&path, content)
            .with_context(|| format!("Failed to write rules config to {}", path.as_ref().display()))?;
    ...
    pub fn load_from_yaml<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        let config: Self = serde_yaml::from_str(&content)
            .with_context(|| format!("Failed to parse YAML config from {}", path.as_ref().display()))?;
    ...
    pub fn validate(&self) -> Result<Vec<String>> {
        ...
        if !self.profiles.contains_key(&self.active_profile) {
            warnings.push(format!("Active profile '{}' not found", self.active_profile));
        }
        ...
        for (profile_name, profile) in &self.profiles {
            if let Some(ref extends) = profile.extends {
                if !self.profiles.contains_key(extends) {
                    warnings.push(format!("Profile '{}' extends non-existent profile '{}'", profile_name, extends));
                }
            }
        }
        ...
        for (profile_name, profile) in &self.profiles {
            for rule_id in &profile.includes {
                if !self.rules.contains_key(rule_id) {
                    warnings.push(format!("Profile '{}' includes unknown rule '{}'", profile_name, rule_id));
                }
            }
            
            for rule_id in &profile.excludes {
                if !self.rules.contains_key(rule_id) {
                    warnings.push(format!("Profile '{}' excludes unknown rule '{}'", profile_name, rule_id));
                }
            }
        }
        ...
        for (rule_id, rule_config) in &self.rules {
            if rule_config.min_confidence < 0.0 || rule_config.min_confidence > 1.0 {
                warnings.push(format!("Rule '{}' has invalid confidence level: {}", rule_id, rule_config.min_confidence));
            }
        }
