...
impl CustomRule {
    ...
    pub fn apply(&self, results: &AnalysisResults) -> Result<Vec<AnalysisError>> {
        ...
        match self.applies_to {
            ...
            RuleTarget::AllMessages => {
                for error in results.get_errors().iter().chain(results.get_warnings().iter()) {
                    if self.matches_text(&error.message)? {
                        if let Some(custom_error) = self.create_custom_error(error)? {
                            new_errors.push(custom_error);
                        }
    ...
    pub fn validate(&self) -> Result<Vec<String>> {
        ...
        if self.confidence < 0.0 || self.confidence > 1.0 {
            issues.push(format!("Confidence must be between 0.0 and 1.0, got {}", self.confidence));
        }
...
impl CustomRulesManager {
    ...
    pub fn load_from_file<P: AsRef<std::path::Path>>(path: P) -> Result<Self> {
        let content = std::fs::read_to_string(&path)
            .with_context(|| format!("Failed to read custom rules from {}", path.as_ref().display()))?;
        
        let rules_data: HashMap<String, CustomRule> = toml::from_str(&content)
            .context("Failed to parse custom rules TOML")?;
    ...
    pub fn add_rule(&mut self, rule: CustomRule) -> Result<()> {
        ...
        if !issues.is_empty() {
            return Err(anyhow::anyhow!("Rule validation failed: {}", issues.join(", ")));
        }
    ...
    pub fn export_to_file<P: AsRef<std::path::Path>>(&self, path: P) -> Result<()> {
        ...
        std::fs::write(&path, content)
            .with_context(|| format!("Failed to write custom rules to {}", path.as_ref().display()))?;
