...
pub use sarif::{SarifLocation, SarifReporter, SarifResult, SarifRule};
...
impl ReportManager {
    ...
    pub fn generate_report(
        ...
        match config.format {
            ReportFormat::Sarif => {
                let reporter =
                    SarifReporter::new("BSL Type Safety Analyzer", env!("CARGO_PKG_VERSION"));
...
mod tests {
    ...
    fn test_report_format_parsing() {
        assert_eq!(
            "sarif".parse::<ReportFormat>().unwrap(),
            ReportFormat::Sarif
        );
