...
class SimpleInteractiveDev {
    ...
    async run() {
        ...
        while (true) {
            ...
            switch (choice.trim()) {
                case '1':
                    await this.runCommand('npm run build:rust:dev', 'Быстрая dev сборка');
                    break;
                    
                case '2':
                    await this.runCommand('npm run build:smart', 'Smart сборка с кешированием');
                    break;
                    
                case '3':
                    await this.runCommand('npm run build:release', 'Release сборка (полная)');
                    break;
                ...
                case '6':
                    await this.runCommand('cargo test', 'Запуск тестов');
                    break;
                    
                case '7':
                    await this.runCommand('cargo clippy', 'Проверка кода');
                    break;
                    
                case '8':
                    await this.runCommand('cargo fmt', 'Форматирование кода');
                    break;
                    
                case '9':
                    await this.runCommand('git status', 'Git статус');
                    break;
                ...
                case '12':
                    await this.runCommand('npm run rebuild:extension', 'Сборка расширения');
                    break;
                    
                case '13':
                    await this.runCommand('npm run version:patch', 'Увеличение patch версии');
                    break;
                    
                case '14':
                    await this.runCommand('npm run version:minor', 'Увеличение minor версии');
                    break;
    ...
    async startWatchMode() {
        ...
        try {
            ...
            const install = await this.ask('Установить chokidar? (y/n): ');
    ...
    async cleanAndRebuild() {
        ...
        const confirm = await this.ask('Продолжить? Это займет время (y/n): ');
        if (confirm.toLowerCase() !== 'y') {
            console.log(c('yellow', 'Отменено'));
            await this.pause();
            return;
        }
        
        await this.runCommand('cargo clean', 'Очистка Cargo');
        await this.runCommand('npm run build:release', 'Полная пересборка');
    }
    ...
    async commitAndPush() {
        ...
        const push = await this.ask('Отправить в удаленный репозиторий? (y/n): ');
        
        if (push.toLowerCase() === 'y' || push.toLowerCase() === 'yes') {
            await this.runCommand('git push', 'Отправка в удаленный репозиторий');
        }
    }
