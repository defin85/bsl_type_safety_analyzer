...
fn main() -> anyhow::Result<()> {
    ...
    match analyzer.analyze_code(&content, "test_semantic.bsl") {
        Ok(()) => {
            ...
            if results.has_errors() || results.has_warnings() {
                ...
                println!("⚠️ Анализ не выявил ожидаемых проблем - проверьте семантический анализатор");
            }
        }
        Err(e) => {
    ...
    match analyzer2.analyze_code(correct_code, "correct.bsl") {
        Ok(()) => {
            ...
            println!("✅ Корректный код: {} ошибок, {} предупреждений", 
                results.error_count(), results.warning_count());
