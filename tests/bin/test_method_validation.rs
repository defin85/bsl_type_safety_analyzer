    ...
    if let Some(ast) = parse_result.ast {
        ...
        match semantic.analyze(&ast) {
            Ok(()) => {
                ...
                println!("\n🎯 Проверка валидации:");
                println!(
                    "   - Найдены ошибки вызовов методов: {}",
                    if method_errors > 0 {
                        "✅ да"
                    } else {
                        "❌ нет"
