    ...
    match analyzer.analyze_code(&content, file_path) {
        Ok(_) => {
            ...
            for func_name in &local_function_names {
                let found_error = errors
                    .iter()
                    .any(|e| e.message.contains(func_name) && e.message.contains("не найдена"));
...
fn test_bsl_parser_directly(content: &str) -> anyhow::Result<()> {
    use bsl_analyzer::bsl_parser::{semantic::SemanticAnalyzer, BslParser};
    ...
    if let Some(ast) = parse_result.ast {
        ...
        for (i, decl) in ast.module.declarations.iter().enumerate() {
            let decl_type = match decl {
                bsl_analyzer::bsl_parser::ast::Declaration::Function(f) => {
                    format!("Функция '{}'", f.name)
                }
                bsl_analyzer::bsl_parser::ast::Declaration::Procedure(p) => {
                    format!("Процедура '{}'", p.name)
                }
                bsl_analyzer::bsl_parser::ast::Declaration::Variable(v) => {
                    format!("Переменные: {:?}", v.names)
                }
            };
        ...
        match semantic_analyzer.analyze(&ast) {
            Ok(_) => {
                ...
                let local_func_errors: Vec<_> = diagnostics
                    .iter()
                    .filter(|d| {
                        d.message.contains("не найдена")
                ...
                println!(
                    "   🔍 Ошибок с локальными функциями: {}",
                    local_func_errors.len()
                );
