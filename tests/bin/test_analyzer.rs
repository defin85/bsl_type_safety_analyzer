...
fn main() -> anyhow::Result<()> {
    ...
    match parse_result.ast {
        Some(ast) => {
            ...
            println!("🌳 AST содержит модуль с {} объявлениями", ast.module.declarations.len());
            ...
            for (i, decl) in ast.module.declarations.iter().enumerate() {
                match decl {
                    bsl_analyzer::bsl_parser::ast::Declaration::Procedure(proc) => {
                        println!("  {}. Процедура: {} (экспорт: {})", i+1, proc.name, proc.export);
                    }
                    bsl_analyzer::bsl_parser::ast::Declaration::Function(func) => {
                        println!("  {}. Функция: {} (экспорт: {})", i+1, func.name, func.export);
                    }
                    bsl_analyzer::bsl_parser::ast::Declaration::Variable(var) => {
                        println!("  {}. Переменная: {:?} (экспорт: {})", i+1, var.names, var.export);
                    }
                }
            }
        }
