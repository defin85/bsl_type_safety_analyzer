...
fn test_generated_keywords(keywords: &GeneratedBslKeywords) -> anyhow::Result<()> {
    ...
    println!("   🔧 Глобальных функций: {}", keywords.global_functions.len());
    println!("   🏛️  Системных объектов: {}", keywords.system_objects.len());
    println!("   📋 Глобальных свойств: {}", keywords.global_properties.len());
    
    // Тестируем основные типы
    println!("\n🧪 Тестирование встроенных типов:");
    let test_types = ["Строка", "Число", "Массив", "ТаблицаЗначений", "Структура", "СписокЗначений"];
    ...
    let test_functions = ["Сообщить", "Формат", "СтрДлина", "ТекущаяДата", "ПрочитатьJSON"];
    ...
    let test_objects = ["Метаданные", "Справочники", "Документы", "ПользователиИнформационнойБазы"];
...
fn test_context_dependent_parsing(keywords: &GeneratedBslKeywords) {
    let test_cases = [
        ("ТаблицаЗначений", BslContext::Expression, true, "В выражении может быть переменной"),
        ("ТаблицаЗначений", BslContext::AfterNew, true, "После 'Новый' является типом"),
        ("Попытка", BslContext::StatementStart, false, "В начале строки - ключевое слово"),
        ("Попытка", BslContext::Expression, false, "В выражении - строгое ключевое слово"),
        ("Сообщить", BslContext::Expression, false, "Глобальная функция, не переменная"),
    ];
    
    for (word, context, expected, description) in &test_cases {
        let result = keywords.can_be_variable(word, *context);
        let status = if result == *expected { "✅" } else { "❌" };
        println!("   {} {} в {:?}: {} ({})", status, word, context, result, description);
    }
}
