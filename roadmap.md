...
**Текущий статус кеша:** 3,923 типа (в основном системные объекты)  
**КРИТИЧЕСКАЯ ПРОБЛЕМА:** Примитивные типы находятся в Language Archive, но парсятся не все
...
# ❌ ОТСУТСТВУЮЩИЕ ПРИМИТИВНЫЕ ТИПЫ:
- Строка (String) ❌ - базовый тип не найден
- Число (Number) ❌ - базовый тип не найден  
- Дата (Date) ❌ - базовый тип не найден
- Булево (Boolean) ❌ - базовый тип не найден
