...
const progressEmitter = new vscode.EventEmitter<IndexingProgress>();

// Функции управления прогрессом индексации
function startIndexing(totalSteps: number = 4) {
...
function updateIndexingProgress(stepNumber: number, stepName: string, progress: number) {
...
function finishIndexing(success: boolean = true) {
    const elapsed = globalIndexingProgress.startTime ? 
        (new Date().getTime() - globalIndexingProgress.startTime.getTime()) / 1000 : 0;
...
async function initializeIndexIfNeeded(context: vscode.ExtensionContext) {
    const config = vscode.workspace.getConfiguration('bslAnalyzer');
    const autoIndexBuild = config.get<boolean>('autoIndexBuild', false);
...
function startLanguageClient(context: vscode.ExtensionContext) {
    ...
    const serverMode = config.get<string>('serverMode', 'stdio');
    ...
    if (serverMode === 'tcp') {
        ...
        serverOptions = {
            command: serverPath,
            args: [],  // No arguments needed for stdio mode
            transport: TransportKind.stdio,
            options: {
                env: { ...process.env, RUST_LOG: 'info' }
            }
    ...
    const clientOptions: LanguageClientOptions = {
        ...
        revealOutputChannelOn: RevealOutputChannelOn.Error, // Показываем канал при ошибках
    ...
    client!.start().then(() => {
        ...
        vscode.window.showWarningMessage(
            `BSL Analyzer LSP server failed to start. Extension features will be limited.`,
            'Show Details'
        ).then(selection => {
            if (selection === 'Show Details') {
                outputChannel.show();
...
function updateStatusBar(text?: string, progress?: IndexingProgress) {
...
function getPlatformDocsArchive(): string {
    const config = vscode.workspace.getConfiguration('bslAnalyzer');
    const userArchive = config.get<string>('platformDocsArchive', '');
...
function getBinaryPath(binaryName: string): string {
    const config = vscode.workspace.getConfiguration('bslAnalyzer');
    const useBundled = config.get<boolean>('useBundledBinaries', true);
...
class BslOverviewProvider implements vscode.TreeDataProvider<BslOverviewItem> {
    ...
    getChildren(element?: BslOverviewItem): Thenable<BslOverviewItem[]> {
        if (!element) {
            ...
            switch (element.contextValue) {
                ...
                case 'config':
                    const config = vscode.workspace.getConfiguration('bslAnalyzer');
                    const configPath = config.get<string>('configurationPath', 'Not configured');
                    return Promise.resolve([
...
class BslDiagnosticsProvider implements vscode.TreeDataProvider<BslDiagnosticItem> {
...
class BslTypeIndexProvider implements vscode.TreeDataProvider<BslTypeItem> {
...
class BslPlatformDocsProvider implements vscode.TreeDataProvider<PlatformDocItem> {
...
class BslActionsWebviewProvider implements vscode.WebviewViewProvider {
...
async function addPlatformDocumentation(provider: BslPlatformDocsProvider): Promise<void> {
    try {
...
async function removePlatformDocumentation(version: string, provider: BslPlatformDocsProvider): Promise<void> {
    ...
    if (choice === 'Remove') {
        try {
            ...
            vscode.window.showInformationMessage(
                `Platform documentation removal for version ${version} - feature coming soon!`
            );
...
async function parsePlatformDocumentation(version: string): Promise<void> {
